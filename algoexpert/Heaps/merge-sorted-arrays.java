...
arrays) {
		...
		for( int arrayIdx = 0; arrayIdx < arrays.size(); 
arrayIdx++){
			smallestItems.add(new Item(arrayIdx, 0, 
arrays.get(arrayIdx).get(0)));
		}
...
arrays.get(smallestItem.arrayIdx).size()-1)
					continue;
...
arrays.get(smallestItem.arrayIdx).get(smallestItem.elementIdx+1)
	...
	static class MinHeap{
		...
		public List<Item> buildHeap(List<Item> array){
			...
			for(int currentIdx = firstParentIdx; currentIdx 
>=0; currentIdx--){
...
heap.get(childTwoIdx).num < heap.get(childOneIdx).num){
			   	...
			   	if( heap.get(idxToSwap).num < 
heap.get(currentIdx).num){
				  	swap(currentIdx, idxToSwap, 
heap);
				  	currentIdx = idxToSwap;
				  	childOneIdx = currentIdx * 2 + 
1;
			    	}    else {
					     return;
				      }
		...
		public void siftUp(int currentIdx, List<Item> heap){
			...
			while( currentIdx>0 && heap.get(currentIdx).num 
< heap.get(parentIdx).num){
